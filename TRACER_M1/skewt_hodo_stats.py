...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 25 00:10:14 2025
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith("csv"):
            ...
            if len(parts) >= 5: # ensuring the file has correct naming format
                ...
                if hour_part == "17" or hour_part == "18":
                    file_path = os.path.join(input_folder, filename)
                    ...
                    # plotting
                    skew.plot(pressure, dry_temp, 'red', label='Temperature', linestyle="dashed", alpha=0.8)
                    skew.plot(pressure, dew_point, 'green', label='Dew Point', linestyle="dashed", alpha=0.8)
                    ...
                    # fiducial lines
                    skew.plot_dry_adiabats(linestyle="dashed", alpha=0.5)
                    skew.plot_moist_adiabats(linestyle="dashed", alpha=0.5)
                    skew.plot_mixing_lines(linestyle="dashed", alpha=0.5)
    
                    skew.plot(pressure, parcel_path, color='black', linestyle="dashed", alpha=0.8)
                    skew.shade_cape(pressure, dry_temp, parcel_path)
                    skew.shade_cin(pressure, dry_temp, parcel_path)
                    
                    # horizontal lines
                    skew.ax.axhline(el[0], color="blue", label="Equilibrium level", linestyle="dashed", alpha=0.8)
    ...
    for file1 in os.listdir(input_folder):
        ...
        if hour_part == "11":
            ...
            for file2 in os.listdir(input_folder):
                ...
                if date_part == date_part2 and (time_part2 == "17" or time_part2 == "18"):
